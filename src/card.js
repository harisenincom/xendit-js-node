...
const CreditCardUtil = require('./utils/credit_card_util');
...
Card.prototype._getTokenizationConfiguration = function (callback) {
  const publicApiKey = this._xendit._getPublishableKey();
  const basicAuthCredentials = 'Basic ' + window.btoa(publicApiKey + ':');
  const xenditBaseURL = this._xendit._getXenditURL();
...
Card.prototype._tokenizeCreditCard = function (tokenizationConfiguration, transactionData, callback) {
  const baseFlexUrl = this._xendit._getEnvironment() === 'PRODUCTION' ? tokenizationConfiguration.flex_production_url : tokenizationConfiguration.flex_development_url;
  const flexUrl = baseFlexUrl + '/cybersource/flex/v1/tokens';
